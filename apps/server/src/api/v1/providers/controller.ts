...
import {BulkServiceSchema, CreateProviderSchema} from "@service-peek/shared";
import {z} from "zod";
import * as serviceRepo from "../../../dal/serviceRepository";
import {providerConnectorFactory} from "../../../bl/providers/providerConnectorFactory";
...
export async function createProvider(req: Request, res: Response) {
    try {
        const providerWithoutID = CreateProviderSchema.parse(req.body);
        const providerID = await providerRepo.createProvider(providerWithoutID);
        res.status(201).json({ success: true, data: { provider: { ...providerWithoutID, id: providerID.lastID }} });
    } catch (error) {
