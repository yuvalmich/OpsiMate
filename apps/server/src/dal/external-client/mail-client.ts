...
export class MailClient {
	...
	private async initTransporter() {
		if (!this.mailerConfig || !this.mailerConfig.enabled || !this.mailerConfig.auth) {
		...
		if (
			...
			logger.warn('MailClient: SMTP settings are not fully configured. Email sending is disabled.');
		}
	}
	...
	private getMailTemplate(mailType: MailType, userName?: string, token?: string): string {
		switch (mailType) {
