...
import { ResetPasswordRow } from './models';
...
export class PasswordResetsRepository {
	...
	async createPasswordResetToken(resetPassword: ResetPasswordType): Promise<void> {
		return runAsync(() => {
			const transaction = this.db.transaction(() => {
				this.db.prepare('DELETE FROM password_resets WHERE user_id = ?').run(resetPassword.userId);
				this.db
