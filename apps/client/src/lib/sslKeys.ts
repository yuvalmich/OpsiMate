...
const logger = new Logger('sslKeys');
...
export async function getSecretsFromServer(): Promise<SecretMetadata[]> {
	try {
		...
		logger.error('Error fetching secrets from server:', error);
		return [];
	}
}
...
export async function createSecretOnServer(
	...
	try {
		...
		logger.error('Error creating secret on server:', error);
		return {
			success: false,
...
export async function updateSecretOnServer(
	...
	try {
		...
		logger.error('Error updating secret on server:', error);
		return {
			success: false,
			error: error instanceof Error ? error.message : 'Unknown error occurred',
		};
...
export async function deleteSecretOnServer(secretId: number): Promise<{ success: boolean; error?: string }> {
	try {
		...
		logger.error('Error deleting secret on server:', error);
		return {
			success: false,
			error: error instanceof Error ? error.message : 'Unknown error occurred',
		};
